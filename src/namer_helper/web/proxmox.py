...
def _next_mp_index(cfg: ProxmoxConfig) -> tuple[bool, int]:
    ok, out = run_remote(cfg, f"pct config {shlex.quote(cfg.lxc_id)}")
    if not ok:
        return False, 0
    used: set[int] = set()
...
def setup_host_mount(
    ...
    ok, out = run_remote(
        cfg,
        f"pct set {shlex.quote(cfg.lxc_id)} -mp{mp_idx} {shlex.quote(f'{host_mp},mp={target}')}",
    )
    steps.append((f"LXC Bind-Mount eintragen (mp{mp_idx})", ok, out))

    fstab_line = f"{nfs_host}:{share} {host_mp} nfs defaults,_netdev 0 0"
    fstab_line_q = shlex.quote(fstab_line)
    ok, out = run_remote(
        cfg,
        f"grep -qxF {fstab_line_q} /etc/fstab || printf '%s\\n' {fstab_line_q} >> /etc/fstab",
    )
...
def teardown_host_mount(
    ...
    ok, out = run_remote(
        cfg,
        f"pct config {shlex.quote(cfg.lxc_id)} | grep -n 'mp=' | grep {host_mp_q} | head -1",
    )
    if ok and out:
        mp_key = out.strip().split(":")[1].split(":")[0].strip() if ":" in out else ""
        if mp_key.startswith("mp"):
            ok2, out2 = run_remote(cfg, f"pct set {shlex.quote(cfg.lxc_id)} --delete {shlex.quote(mp_key)}")
    ...
    ok, out = run_remote(cfg, f"umount {host_mp_q} 2>&1 || true")
    ...
    ok, out = run_remote(
        cfg,
        f"grep -vxF {fstab_line_q} /etc/fstab > /etc/fstab.namer-helper.tmp && mv /etc/fstab.namer-helper.tmp /etc/fstab",
    )
