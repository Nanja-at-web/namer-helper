...
def test_ignored_file_filters_metadata_and_zero_byte_files(tmp_path):
    ...
    dot_at_meta_dir = tmp_path / ".@__thumb"
    dot_at_meta_dir.mkdir()
    nested_meta = dot_at_meta_dir / "preview.mp4"
    nested_meta.write_bytes(b"data")
    ...
    assert _is_ignored_file(nested_meta) is True
